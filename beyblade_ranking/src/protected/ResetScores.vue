<template>
  <OrganizerGate>
    <div class="reset-scores p-4">
      ...
      <div class="flex flex-column md:flex-row justify-content-center gap-4">
        <Card class="w-full md:w-auto" style="max-width: 32rem;">
          ...
          <template #content>
            ...
            <Message
              v-if="playerResult.message"
              :severity="playerResult.severity"
              :closable="false"
              class="mb-4"
            >
        ...
        <Card class="w-full md:w-auto" style="max-width: 32rem;">
          ...
          <template #content>
            ...
            <Message
              v-if="clanResult.message"
              :severity="clanResult.severity"
              :closable="false"
              class="mb-4"
            >
              {{ clanResult.message }}
      ...
      <Dialog
        v-model:visible="playerConfirmVisible"
        modal
        header="Confirm player score reset"
        :style="{ width: '28rem' }"
      >
        <p class="m-0 mb-3">
        ...
        <template #footer>
          <Button
            ...
            severity="secondary"
            text
            @click="playerConfirmVisible = false"
          />
          <Button
            label="Yes, reset everything"
            severity="danger"
            :loading="playerResetting"
            @click="resetPlayerScores"
          />
        </template>
      </Dialog>

      <Dialog
        v-model:visible="clanConfirmVisible"
        modal
        header="Confirm clan score reset"
        :style="{ width: '28rem' }"
      >
        <p class="m-0 mb-3">
        ...
        <template #footer>
          <Button
            ...
            severity="secondary"
            text
            @click="clanConfirmVisible = false"
          />
          <Button
            label="Yes, reset everything"
            severity="danger"
            :loading="clanResetting"
            @click="resetClanScores"
          />
        </template>
      </Dialog>
    </div>
...
async function resetPlayerScores() {
  ...
  try {
    ...
    playerResult.value = {
      message: 'Could not reset player scores. Try again.',
      severity: 'error',
    }
  } finally {
...
async function resetClanScores() {
  ...
  try {
    ...
    clanResult.value = {
      message: 'Could not reset clan scores. Try again.',
      severity: 'error',
    }
  } finally {
