<template>
  <div class="leaderboard-container">
    ...
    <div class="flex justify-content-center align-items-end gap-4 mb-5">
      <Card
        v-for="clan in clans"
        :key="clan.name"
        class="clan-card"
        :class="{ 'top-clan': clan.rank === 1 }"
      >
        <template #content>
          <div class="flex flex-column align-items-center">
            <h3>{{ clan.rank }}: {{clan.name}}</h3>
            <p class="text-secondary">{{ clan.points }} points</p>
          </div>
        </template>
      </Card>
    </div>
...
onMounted(() => {
  ...
  clans.value = [second, first, third];
});
</script>
